package com.thundersoft.mvvm.app.customview;
...
import com.thundersoft.mvvm.common.lib.entity.AudioLrcBean;
import com.thundersoft.mvvm.common.lib.utils.DensityUtils;
import com.thundersoft.mvvm.common.lib.utils.MusicUtils;
...
public class MusicLrcView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mLrcTextSize = typedArray.getDimension(R.styleable.MusicLrcView_lrcTextSize,
                DensityUtils.dp2px(context, MusicConstants.UsbMusicLrc.LRC_TEXT_SIZE));
        ...
        mTouchDelay = typedArray.getInt(R.styleable.MusicLrcView_lrcTouchDelay,
                MusicConstants.UsbMusicLrc.INDICATOR_TOUCH_DELAY);
        ...
        mIndicatorLineWidth = typedArray.getDimension(R.styleable.MusicLrcView_indicatorLineHeight,
                DensityUtils.dp2px(context, MusicConstants.UsbMusicLrc.INDICATOR_LINE_WIDTH));
        ...
        mIndicatorMargin = typedArray.getDimension(R.styleable.MusicLrcView_indicatorStartEndMargin,
                DensityUtils.dp2px(context, MusicConstants.UsbMusicLrc.ICON_LINE_GAP));
    ...
    private void setupConfigs(Context context) {
        ...
        mOverScroller.setFriction(MusicConstants.UsbMusicLrc.OVER_SCROLLER);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (changed) {
            ...
            mPlayRect.top = (int) (getHeight() / MusicConstants.UsbMusicLrc.PLAY_RECT_TOP
                    - mIconHeight / MusicConstants.UsbMusicLrc.PLAY_RECT_TOP);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mTextPaint.setTextSize(mLrcTextSize);
        mTextPaint.setTextAlign(Paint.Align.CENTER);
        float lrcWidth = getLrcWidth() / MusicConstants.UsbMusicLrc.LRC_HEIGHT + getPaddingLeft();
        float lrcHeight = getLrcHeight() / MusicConstants.UsbMusicLrc.LRC_HEIGHT;
        for (int i = 0; i < getLrcCount(); i++) {
            if (i > 0) {
                lrcHeight += (getTextHeight(i - MusicConstants.UsbMusicLrc.LINE_POSITION)
                        + getTextHeight(i)) / MusicConstants.UsbMusicLrc.LRC_HEIGHT + mLrcLineSpaceHeight;
            }
            if (mCurrentLine == i) {
        ...
        if (mIsShowTimeIndicator) {
            ...
            canvas.drawLine(mPlayRect.right + mIconLineGap, getHeight()
                            / MusicConstants.UsbMusicLrc.LRC_HEIGHT, getWidth() - timeWidth
                            * MusicConstants.UsbMusicLrc.TIME_WIDTH - LINE_STOP_MARGIN_X,
                    getHeight() / MusicConstants.UsbMusicLrc.LRC_HEIGHT, mIndicatorPaint);
            int baseX = (int) (getWidth() - timeWidth * MusicConstants.UsbMusicLrc.BASE_X);
            float baseline = getHeight() / MusicConstants.UsbMusicLrc.LRC_HEIGHT - (mIndicatorPaint.descent()
                    - mIndicatorPaint.ascent()) / MusicConstants.UsbMusicLrc.PLAY_RECT_TOP
    ...
    private void drawLrc(Canvas canvas, float x, float y, int i) {
        ...
        if (staticLayout == null) {
            ...
            staticLayout = new StaticLayout(text, mTextPaint, getLrcWidth(),
                    Layout.Alignment.ALIGN_NORMAL, MusicConstants.UsbMusicLrc.STATIC_LAYOUT_SPACING_X,
                    MusicConstants.UsbMusicLrc.STATIC_LAYOUT_SPACING_Y, false);
            mLrcMap.put(text, staticLayout);
        }
        canvas.save();
        canvas.translate(x, y - staticLayout.getHeight() / MusicConstants.UsbMusicLrc.LRC_HEIGHT - mOffset);
    ...
    private void drawEmptyText(Canvas canvas) {
        ...
        StaticLayout staticLayout = new StaticLayout(mDefaultContent, mTextPaint, getLrcWidth(),
                Layout.Alignment.ALIGN_NORMAL, MusicConstants.UsbMusicLrc.STATIC_LAYOUT_SPACING_X,
                MusicConstants.UsbMusicLrc.STATIC_LAYOUT_SPACING_Y, false);
        canvas.translate(getLrcWidth() / MusicConstants.UsbMusicLrc.LRC_HEIGHT + getPaddingLeft(),
                getLrcHeight() / MusicConstants.UsbMusicLrc.LRC_HEIGHT);
        staticLayout.draw(canvas);
        canvas.restore();
    }
    ...
    private int getUpdateTimeLinePosition(long time) {
        ...
        for (int i = 0; i < getLrcCount(); i++) {
            ...
            if (time >= lrc.getTime()) {
                if (i == getLrcCount() - MusicConstants.UsbMusicLrc.LRC_COUNT_INDEX) {
                    linePos = getLrcCount() - MusicConstants.UsbMusicLrc.LRC_COUNT_INDEX;
                } else if (time < mLrcData.get(i + MusicConstants.UsbMusicLrc.LRC_COUNT_INDEX).getTime()) {
                    linePos = i;
                    break;
                }
            }
    ...
    private void scrollToPosition(int linePosition) {
        ...
        animator.setDuration(MusicConstants.UsbMusicLrc.LRC_ANIMATOR_DURATION);
        animator.start();
    }
    ...
    private float getItemOffsetY(int linePosition) {
        ...
        for (int i = 1; i <= linePosition; i++) {
            tempY += (getTextHeight(i - MusicConstants.UsbMusicLrc.LRC_COUNT_INDEX)
                    + getTextHeight(i)) / MusicConstants.UsbMusicLrc.PLAY_RECT_TOP + mLrcLineSpaceHeight;
        }
        return tempY;
    }
    ...
    private boolean overScrolled() {
        return mOffset > getItemOffsetY(getLrcCount() - MusicConstants.UsbMusicLrc.LRC_COUNT_INDEX)
                || mOffset < 0;
    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsDragging) {
                    float maxHeight = getItemOffsetY(getLrcCount()
                            - MusicConstants.UsbMusicLrc.LRC_COUNT_INDEX);
                    if (mOffset < 0 || mOffset > maxHeight) {
                        moveY /= MusicConstants.UsbMusicLrc.LRC_MOVE_Y;
                    }
                    mOffset -= moveY;
                    mLastMotionY = event.getY();
                    invalidateView();
    ...
    private void handleActionUp(MotionEvent event) {
        ...
        if (overScrolled() && mOffset < 0) {
            scrollToPosition(MusicConstants.UsbMusicLrc.SCROLL_TO_POSITION);
        ...
        if (overScrolled() && mOffset > getItemOffsetY(getLrcCount()
                - MusicConstants.UsbMusicLrc.LRC_COUNT_INDEX)) {
            scrollToPosition(getLrcCount() - MusicConstants.UsbMusicLrc.LRC_COUNT_INDEX);
            if (mIsAutoAdjustPosition) {
                ViewCompat.postOnAnimationDelayed(this, mScrollRunnable, mTouchDelay);
            }
            return;
        ...
        if (absVelocity > mMinimumFlingVelocity) {
            mOverScroller.fling(MusicConstants.UsbMusicLrc.SCROLL_TO_POSITION,
                    (int) mOffset, MusicConstants.UsbMusicLrc.SCROLL_TO_POSITION,
                    (int) (-velocity),
                    MusicConstants.UsbMusicLrc.SCROLL_TO_POSITION,
                    MusicConstants.UsbMusicLrc.SCROLL_TO_POSITION, MusicConstants.UsbMusicLrc.SCROLL_TO_POSITION,
                    (int) getItemOffsetY(getLrcCount() - MusicConstants.UsbMusicLrc.LRC_COUNT_INDEX),
                    MusicConstants.UsbMusicLrc.SCROLL_TO_POSITION,
                    (int) getTextHeight(MusicConstants.UsbMusicLrc.SCROLL_TO_POSITION));
            invalidateView();
        }
    ...
    public void resetView(String defaultContent) {
        ...
        mCurrentLine = MusicConstants.UsbMusicLrc.SCROLL_TO_POSITION;
        mOffset = MusicConstants.UsbMusicLrc.SCROLL_TO_POSITION;
        mIsUserScroll = false;
        mIsDragging = false;
